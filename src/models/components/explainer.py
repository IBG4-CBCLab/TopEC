...
class Explainer(torch.nn.Module):
    ...
    @torch.no_grad()
    def get_initial_prediction(self, x: Tensor, pos: Tensor, edge_index: Tensor,
                               batch: Optional[Tensor] = None, **kwargs):
        if batch is not None:
            _, out = self.model(x, pos, edge_index, batch, **kwargs)
        else:
            _, out = self.model(x, pos, edge_index, **kwargs)
